...
var themeColors = [ $warning,$primary,$success, $info ,  $danger];
...
export class ApexComponent implements OnInit {
  ...
  constructor(private DataTabRService: DataTabRemService,private DataTabSService: DataTabSiteService,private DataTabQService: DataTabQuaiService) {
    ...
    this.lineChartOptions = {
      ...
      xaxis: {
       ...
       categories:this.dataTR ,
      },
      yaxis: {
        title: {offsetY : -20 ,
          text: 'Temps en minute'
        }},
 ...
 // -----------------------------------------------------------------------------------------------------
    ...
    this.pieChartOptions1 = {
      ...
      colors: themeColors,
      labels: [' Trés urgent', 'Moyennement urgent ', 'Peu urgent '],
      series: this.dataURG ,
      legend: {
        itemMargin: {
          horizontal: 2
  ...
  dataRetard :any=[];
  dataListQuai :any=[];
  dataNbrRemQuaiRds :any =[];
  dataNbrRemQuaiBnA :any =[];
  dataListRem : any=[];
 ...
 // -----------------------------------------------------------------------------------------------------
    ...
    //extraction retard des remorques 
    ...
    this.DataTabSService.getSite().subscribe(
      data => { console.log(data) ;
         ...
         data.forEach(element => {
          ...
          this.dataRetard.push(60*(new Date (element['timeRAPL']).getHours()  - new Date (element['timeSPL']).getHours() ) + (new Date (element['timeRAPL']).getMinutes() ) - (new Date (element['timeSPL']).getMinutes() ))
    ...
    this.DataTabSService.getSite().subscribe(
      data => { console.log(data) ;
        ...
        data.forEach(element => {
          ...
          this.dataTE.push((new Date (element['timeSPL']).getHours())+"."+(new Date (element['timeSPL']).getMinutes()));              
...
//extraction NOMBRE DE REMORQUE PAR QUAI Rades 
...
this.DataTabQService.getQuaiList().subscribe(
  data => { console.log(data) ;
    ...
    for(let i=0;i<3;i++){
      ...
      this.dataListQuai.push(data[i]);  
      
      
      this.DataTabRService.getNbRemByQuai(data[i]["idQuai"]).subscribe(
        data => { console.log("yes"),console.log(data) ;
        
          this.dataNbrRemQuaiRds.push(data) ;
            this.IsDataReady = true;
        }, error => { console.log(error) })
   
      this.IsDataReady = true;
  }
   //extraction NOMBRE DE REMORQUE PAR QUAI BenArous 
    for(let i=5 ;i>2 ;i--){
    ...
    this.DataTabRService.getNbRemByQuai(data[i]["idQuai"]).subscribe(
      data => { console.log("yes"),console.log(data) ;
        ...
        this.dataNbrRemQuaiBnA.push(data) ;
          this.IsDataReady = true;
      }, error => { console.log(error) })
...
}, error => { console.log("non") })
   
  console.info('dataListQuai');console.info(this.dataListQuai);

  console.info('dataNbrRemQuaiBnA');console.info(this.dataNbrRemQuaiBnA);

        console.info('dataNbrRemQuaiRds');console.info(this.dataNbrRemQuaiRds);
        

       
   this.DataTabRService.getNburg(1).subscribe(
     data => {console.log("urg1"),console.log(data) }
   )
        
 //extraction NOMBRE DE REMORQUE AYANT degree d'urgence 


    for(let i=1;i<4;i++){

      this.DataTabRService.getNburg(i).subscribe(
        data => { console.log("yes"),console.log(data) ;
        
          this.dataURG.push(data) ;
            this.IsDataReady = true;
        }, error => { console.log(error) })           
  ...
  console.info('dataURG');console.info(this.dataURG);
